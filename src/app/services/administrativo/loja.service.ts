...
export class LojaService {
  ...
  atualizarLoja(id: string, loja: Loja): Observable<Loja> {
    const url = `${this.apiURL}/${id}`;
    return this.http.put<Loja>(url, loja).pipe(
      map((response) => response),
      catchError((error) => {
  ...
  deleteLojaById(id: string): Observable<void> {
    const url = `${this.apiURL}/${id}`;
    return this.http.delete<void>(url).pipe(
      catchError((error) => {
        let errorMessage = 'Erro ao deletar a loja.';
  ...
  buscarLojasPorNome(nome: string): Observable<Loja[]> {
    const url = `${this.apiURL}/search/${encodeURIComponent(nome)}`;
    return this.http.get<Loja[]>(url).pipe(
      map((response) => response),
      catchError((error) => {
        let errorMessage = 'Erro ao buscar lojas por nome.';
        if (error.error instanceof ErrorEvent) {
          errorMessage = `Erro: ${error.error.message}`;
        } else if (error.status) {
          errorMessage = `Erro no servidor: ${error.status} - ${error.message}`;
